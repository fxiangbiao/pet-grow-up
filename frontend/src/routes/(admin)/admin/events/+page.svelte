<script lang="ts">
  ...
  onMount(() => { load(); });
  async function load() { loading = true; try { pageData = await adminApi.listEvents(filter); } catch (e: any) { toastStore.error(e.message || '加载失败'); } finally { loading = false; } }
  function applyFilter(updates: any) { filter = { ...filter, ...updates, page: 1 }; load(); }
  function goPage(p: number) { filter = { ...filter, page: p }; load(); }
  async function handleDelete(id: number) { try { await adminApi.deleteEvent(id); toastStore.success('事件已删除'); confirmDelete = null; load(); } catch (e: any) { toastStore.error(e.message || '删除失败'); } }
  let totalPages = $derived(Math.ceil(pageData.total / pageData.size));
</script>

<div>
  <div class="flex items-center justify-between mb-4">
    <h2 class="text-xl font-bold text-gray-800">🎲 随机事件管理</h2>
    <button onclick={() => goto('/admin/events/new')} class="px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 transition">➕ 新建事件</button>
  </div>
  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    ...
    {:else if pageData.items.length > 0}
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b"><tr>
          ...
          <th class="text-center px-4 py-2 font-medium text-gray-500">触发概率</th>
          <th class="text-center px-4 py-2 font-medium text-gray-500">奖励⚡</th>
          <th class="text-center px-4 py-2 font-medium text-gray-500">连击</th>
        ...
        <tbody class="divide-y">
          {#each pageData.items as item}
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-xs text-gray-500 font-mono">{item.eventKey}</td>
              <td class="px-4 py-2"><span class="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">{EVENT_TYPE_LABELS[item.eventType] || item.eventType}</span></td>
              <td class="px-4 py-2 text-center">{item.triggerChance}%</td>
              <td class="px-4 py-2 text-center text-xs">⚡{item.rewardEnergy}</td>
              <td class="px-4 py-2 text-center text-xs">{item.minStreak || '-'}</td>
              <td class="px-4 py-2 text-center">
                <button onclick={() => goto('/admin/events/' + item.id)} class="text-indigo-600 hover:text-indigo-800 text-xs mr-2">编辑</button>
    ...
    {:else}<div class="text-center text-gray-400 py-12">暂无事件</div>{/if}
  </div>
</div>
<ConfirmModal show={confirmDelete !== null} title="确认删除" confirmText="删除" message="确定要删除该事件吗？"
